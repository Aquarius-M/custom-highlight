...
class Type5ColorShow implements DocumentColorProvider {
    ...
    provideDocumentColors(document: TextDocument): ProviderResult<ColorInformation[]> {
        ...
        const regex = /rgbo\(\s*(\d{1,3}),\s*(\d{1,3}),\s*(\d{1,3}),\s*(\d?\.?\d+)\)/gi;
    ...
    provideColorPresentations(color: Color): ProviderResult<ColorPresentation[]> {
        ...
        return [new ColorPresentation(`RGBO(${colorLabel.toLocaleUpperCase()})`)];
    }
}
export default Type5ColorShow;
